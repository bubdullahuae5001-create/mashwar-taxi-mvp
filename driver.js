...
async function routeUser(){
 ...
 if(d.approval_status==='pending'||vehicle?.approval_status==='pending'){
 ...
 if(['suspended','rejected'].includes(d.approval_status)){showOnly('suspendedView');return}
 if(d.approval_status!=='approved'){showOnly('pendingView');return}
...
async function refreshDriver(){const {data:u}=await sb.auth.getUser();if(!u.user)return;const {data:d}=await sb.from('drivers').select('*').eq('user_id',u.user.id).maybeSingle();if(!d)return;driver=d;if(d.approval_status!=='approved'){await routeUser();return}renderDriver();await Promise.all([loadOffers(),loadActiveRide()])}
